...
const Indbox = () => {
  ...
  const GetData = useCallback(async () => {
    try {
      setloader(true);
      let res = await fetch(
        `https://mailclient-dd8ea-default-rtdb.firebaseio.com/${mail}indbox.json`
      );
  ...
  const DeleteHandler = async (val) => {
    ...
    try {
      const response = await fetch(
        `https://mailclient-dd8ea-default-rtdb.firebaseio.com/${mail}trashMail.json`,
        {
          method: "POST",
    ...
    try {
      const res = await fetch(
        `https://mailclient-dd8ea-default-rtdb.firebaseio.com/${mail}indbox/${val.id}.json`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
