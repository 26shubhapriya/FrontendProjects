...
const AuthForm = () => {
  ...
  const onSubmitHandler=(e)=>{
    ...
    setLoading(true);
    if(isLogin){
      fetch("https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDFUhVsBRm5OtimsKO-HwtuMduXPEcyPzk",{
        method:'POST',
        body: JSON.stringify({
          email: enteredEmail,
      ...
      fetch("https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyDFUhVsBRm5OtimsKO-HwtuMduXPEcyPzk",{
        method:'POST',
        body: JSON.stringify({
          email: enteredEmail,
          password: enteredPassword,
