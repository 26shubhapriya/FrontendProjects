  ...
  const updateHandler = async (e) => {
    ...
    try {
      const res = await fetch(
        "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyBMVblYc-kgZPBCYozHEju1An9kxNT_kdM",
        {
          method: "POST",
          body: JSON.stringify({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(
          "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyBMVblYc-kgZPBCYozHEju1An9kxNT_kdM",
          {
            method: "POST",
            body: JSON.stringify({
              idToken: token,
